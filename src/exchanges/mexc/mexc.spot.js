 ...
 * @param {ArrayBuffer} arrayBuffer - Raw protobuf data from WebSocket
 * @returns {Object|null} Decoded message or null if error
 */
function decodeMexcProtobuf(arrayBuffer) {
  try {
 ...
 * @param {Object} message - Decoded PushDataV3ApiWrapper message
 * @param {string} pair - Trading pair symbol
 * @returns {Array} Array of formatted trades
 */
function extractTradesFromProtobuf(message, pair) {
 ...
 * @returns {boolean} true if handled, falsy otherwise
 */
function handleSpotJsonMessage(json) {
